...
class UtilsListener(JavaParserListener):
    ...
    def enterClassCreatorRest(self, ctx: JavaParser.ClassCreatorRestContext):
        if self.current_method is not None:
            #type name classname
            self.package.classes[self.current_class_identifier].class_invocations.append(
                ('method', self.current_method.name, ctx.parentCtx.getText())
            )
        elif len(self.current_field_var_ctxs) > 0:
            self.package.classes[self.current_class_identifier].class_invocations.append(
                ('field', self.current_field_ids[-1], ctx.parentCtx.getText())
            )
    ...
    #             self.current_method.return_statement = ctx
    #
    ...
    def enterMethodCall(self, ctx: JavaParser.MethodCallContext):
        if self.current_method is not None:
            if ctx.IDENTIFIER() is not None:
                self.current_method.method_invocations.append(ctx.IDENTIFIER().getText())
